...
const coffee = new CreateMenuItem("Coffee", 2.99, "Drinks");
const sandwhich = new CreateMenuItem("Sandwhich", 2.99, "Lunch");
const byo = new CreateMenuItem("Build Your Own", 9.99, "Breakfast");
byo.options = ["Pancakes", "Sausage", "Bacon", "Hashbrowns", "Eggs", "Toast"];
...
burger.discount = function (disc) {
  let string = disc.toLowerCase();
  switch (string) {
    case "teacher":
...
reviews.push({
  ...
  rating: 5,
  feedback:
    "Nothing but great things to say about the food hear. Great Prices, Better Service. I can see my self eating here every single day of my life.",
});
...
function getReviewByIndex(reviews, i) {
  return `\n${reviews[i].name} gave the restaurant a ${reviews[i].rating} star review, and their feedback was:\n"${reviews[i].feedback}"`;
}
...
function getLastReview(arr) {
  ...
  return `\n${arr[length].name} gave the restaurant a ${arr[length].rating}, and their feedback was:\n"${arr[length].feedback}"`;
}
...
function getReviewByRating(arr, rating) {
  return arr.filter((rate) => Math.floor(rate.rating) === Math.floor(rating));
}
...
function getLongReviews(arr) {
  return arr.filter((element) => element.feedback.split(" ").length > 15);
}
